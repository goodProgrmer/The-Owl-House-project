...
def get_public_key(sock,key):
    """save the key of the given socket and send confirmation meseg if when done (if it there is no saved key for this socket by this time).
    ...
    global sending_sock_key
    try:
        sending_sock_key[sock]
    except:
        sending_sock_key[sock]=RSA.import_key(key)
        sendMesegTCP(sock,"CONNECTION SECURED")
